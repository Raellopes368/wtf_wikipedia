...
test('basic-citation', t => {
  let str = `Emery is a vegetarian,<ref>{{cite web|title=The princess of pot|url=http://thewalrus.ca/the-princess-of-pot/}}</ref>`;
  let arr = wtf(str)
    .citations()
    .map(c => c.json());
...
test('complex-citation', t => {
  let str = `Emery is a vegetarian,<ref name="fun">{{ cite web|foo =    bar
...
test('multiple-citations', t => {
  let str = `hello {{citation |url=cool.com/?fun=yes/   }}{{CITE book |title=the killer and the cartoons }}`;
  let arr = wtf(str)
    .citations()
    .map(c => c.json());
...
test('crazy-long-citations', t => {
  let str = `{{Begriffsklärungshinweis}}
...
test('inline-test', t => {
  let str = ` {{cite gnis|id=1145117|name=Little Butte Creek|entrydate=November 28, 1980|accessdate=September 26, 2009}}
