...
test('generic-list', t => {
  let str = `{{Portal bar|portal 1|portal 2}}`;
  let doc = wtf(str);
  let obj = doc.templates(0);
...
test('redirect-list', t => {
  let str = `{{Redirect|City of Toronto|the municipal government|Municipal government of Toronto|the historical part of the city prior to the 1998 amalgamation|Old Toronto}}`;
  let doc = wtf(str);
  let obj = doc.templates(0);
...
test('templates-in-templates', t => {
  let str = `{{marriage|[[Mileva Marić]]<br>|1903|1919|end=div}}<br />{{nowrap|{{marriage|[[Elsa Löwenthal]]<br>|1919|1936|end=died}}<ref>{{cite book |editor-last=Heilbron |editor-first=John L. |title=The Oxford Companion to the History of Modern Science |url=https://books.google.com/books?id=abqjP-_KfzkC&pg=PA233 |date=2003 |publisher=Oxford University Press |isbn=978-0-19-974376-6 |page=233}}</ref>{{sfnp|Pais|1982|p=301}}}}`;
  let ref = wtf(str)
    .citations(0)
    .json();
  ...
  let templates = wtf(str).templates();
...
test('support-nowrap-in-infobox', t => {
  ...
  `;
  let infobox = wtf(str).infoboxes(0);
  let data = infobox.json();
...
test('inline-templates', t => {
  let str = `he married {{marriage|[[Elsa Löwenthal]]<br>|1919|1936|end=died}} in Peterburough`;
  let doc = wtf(str);
  t.equal(
    doc.text(),
...
test('three-layer-templates', t => {
  let str = `she married {{nowrap| {{nowrap| {{marriage|Johnny-boy}} }}}}`;
  let doc = wtf(str);
